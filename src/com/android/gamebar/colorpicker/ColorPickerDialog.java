...
public class ColorPickerDialog extends DialogFragment implements ColorPickerView.OnColorChangedListener, TextWatcher {
  ...
  @Override public Dialog onCreateDialog(Bundle savedInstanceState) {
    ...
    if (dialogType == TYPE_CUSTOM && getArguments().getBoolean(ARG_ALLOW_PRESETS)) {
      neutralButtonStringRes = (presetsButtonStringRes != 0 ? presetsButtonStringRes : R.string.gamebar_presets);
    } else if (dialogType == TYPE_PRESETS && getArguments().getBoolean(ARG_ALLOW_CUSTOM)) {
      neutralButtonStringRes = (customButtonStringRes != 0 ? customButtonStringRes : R.string.gamebar_custom);
    } else {
  ...
  View createPickerView() {
    View contentView = View.inflate(getActivity(), R.layout.gamebar_dialog_color_picker, null);
    colorPicker = (ColorPickerView) contentView.findViewById(R.id.gamebar_color_picker_view);
    ColorPanelView oldColorPanel = (ColorPanelView) contentView.findViewById(R.id.gamebar_color_panel_old);
    newColorPanel = (ColorPanelView) contentView.findViewById(R.id.gamebar_color_panel_new);
    ImageView arrowRight = (ImageView) contentView.findViewById(R.id.gamebar_arrow_right);
    hexEditText = (EditText) contentView.findViewById(R.id.gamebar_hex);
  ...
  View createPresetsView() {
    View contentView = View.inflate(getActivity(), R.layout.gamebar_dialog_presets, null);
  ...
  void createColorShades(@ColorInt final int color) {
    ...
    if (shadesLayout.getChildCount() != 0) {
      for (int i = 0; i < shadesLayout.getChildCount(); i++) {
        ...
        final ColorPanelView cpv = (ColorPanelView) layout.findViewById(R.id.gamebar_color_panel_view);
        ImageView iv = (ImageView) layout.findViewById(R.id.gamebar_color_image_view);
    ...
    final int horizontalPadding = getResources().getDimensionPixelSize(R.dimen.gamebar_item_horizontal_padding);

    for (final int colorShade : colorShades) {
      int layoutResId;
      if (colorShape == ColorShape.SQUARE) {
        layoutResId = R.layout.gamebar_color_item_square;
      } else {
        layoutResId = R.layout.gamebar_color_item_circle;
      }

      final View view = View.inflate(getActivity(), layoutResId, null);
      final ColorPanelView colorPanelView = (ColorPanelView) view.findViewById(R.id.gamebar_color_panel_view);
      ...
      colorPanelView.setOnClickListener(new View.OnClickListener() {
        @Override public void onClick(View v) {
          ...
          for (int i = 0; i < shadesLayout.getChildCount(); i++) {
            FrameLayout layout = (FrameLayout) shadesLayout.getChildAt(i);
            ColorPanelView cpv = (ColorPanelView) layout.findViewById(R.id.gamebar_color_panel_view);
            ImageView iv = (ImageView) layout.findViewById(R.id.gamebar_color_image_view);
            iv.setImageResource(cpv == v ? R.drawable.gamebar_preset_checked : 0);
  ...
  public static final class Builder {
    ...
    ColorPickerDialogListener colorPickerDialogListener;
    @StringRes int dialogTitle = R.string.gamebar_default_title;
    @StringRes int presetsButtonText = R.string.gamebar_presets;
    @StringRes int customButtonText = R.string.gamebar_custom;
    @StringRes int selectedButtonText = R.string.gamebar_select;
