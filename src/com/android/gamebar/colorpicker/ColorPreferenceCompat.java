...
public class ColorPreferenceCompat extends Preference implements ColorPickerDialogListener {
  ...
  private void init(AttributeSet attrs) {
    ...
    showDialog = a.getBoolean(R.styleable.ColorPreference_gamebar_showDialog, true);
    //noinspection WrongConstant
    dialogType = a.getInt(R.styleable.ColorPreference_gamebar_dialogType, ColorPickerDialog.TYPE_PRESETS);
    colorShape = a.getInt(R.styleable.ColorPreference_gamebar_colorShape, ColorShape.CIRCLE);
    allowPresets = a.getBoolean(R.styleable.ColorPreference_gamebar_allowPresets, true);
    allowCustom = a.getBoolean(R.styleable.ColorPreference_gamebar_allowCustom, true);
    showAlphaSlider = a.getBoolean(R.styleable.ColorPreference_gamebar_showAlphaSlider, false);
    showColorShades = a.getBoolean(R.styleable.ColorPreference_gamebar_showColorShades, true);
    previewSize = a.getInt(R.styleable.ColorPreference_gamebar_previewSize, SIZE_NORMAL);
    final int presetsResId = a.getResourceId(R.styleable.ColorPreference_gamebar_colorPresets, 0);
    dialogTitle = a.getResourceId(R.styleable.ColorPreference_gamebar_dialogTitle, R.string.gamebar_default_title);
    if (presetsResId != 0) {
    ...
    if (colorShape == ColorShape.CIRCLE) {
      setWidgetLayoutResource(
          previewSize == SIZE_LARGE ? R.layout.gamebar_preference_circle_large : R.layout.gamebar_preference_circle);
    } else {
      setWidgetLayoutResource(
          previewSize == SIZE_LARGE ? R.layout.gamebar_preference_square_large : R.layout.gamebar_preference_square);
    }
    a.recycle();
  }
  ...
  @Override public void onBindViewHolder(PreferenceViewHolder holder) {
    ...
    ColorPanelView preview = (ColorPanelView) holder.itemView.findViewById(R.id.gamebar_preference_preview_color_panel);
    if (preview != null) {
